...
class MainDialog(object):
    ...
    def __init__(self):
        ...
        self.w.doGlyphMarksCheckBox = dialogKit.CheckBox((435, 212, -12, 20), "Glyph Marks", value=True)
        ...
        self.w.doConvertKerningToMMK = dialogKit.CheckBox((435, 300, -12, 40), "Export kerning to MMK", value=False)
    ...
    def okCallback(self, sender):
        ...
        doMMK = self.w.doConvertKerningToMMK.get()
        formatVersion = 2
        ...
        if self.mode == "export":
            for path, font in self.files.items():
                exportUFO(font, newFile, doInfo=doInfo, doKerning=doKerning, doGroups=doGroups, doLib=doLib, doFeatures=doFeatures,
                    doHints=doGlyphHints, doMarks=doGlyphMarks, doMasks=doGlyphMasks, glyphs=self.glyphs, formatVersion=formatVersion, doMMK = doMMK)
        else:
...
def getKernStrukt(font):
    ...
    for index, glyphClass in enumerate(font.naked().classes):
        if glyphClass.startswith("_"):
            # print glyphClass, 'KERN class'
            kernClasses.append(glyphClass)
            kernClassesToBooleans[glyphClass] = (font.naked().GetClassLeft(index), font.naked().GetClassRight(index))
        else:
            # print glyphClass, 'FEATURE class'
            feaClasses.append(glyphClass)
    return [kernClasses, kernClassesToBooleans, feaClasses]


def exportUFO(font, newFile=True, doInfo=True, doKerning=True, doGroups=True, doLib=True, doFeatures=True,
    doHints=False, doMarks=True, doMasks=True, glyphs=None, formatVersion=2, doMMK = False):
    # get the UFO path
    ...
    if doMMK:
        ...
        print 'Saving KernDump to', kernDumpPath
        kern_strukt = getKernStrukt(font)

        filekern = open(kernDumpPath, 'w')
        pickle.dump(kern_strukt, filekern)
        filekern.close()

    font.writeUFO(path=ufoPath, doProgress=True, doHints=doHints, doInfo=doInfo,
...
def importUFO(ufoPath, newFile=True, saveFile=True, closeFile=True, doInfo=True, doKerning=True, doGroups=True,
    ...
    fl.ifont = font.fontIndex
    # read the UFO
    font.readUFO(ufoPath, doProgress=True, doHints=doHints, doInfo=doInfo, doKerning=doKerning,
